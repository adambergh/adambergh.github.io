...
// $('.ui.menu:first').append('<a class="item toggle"><i class="icon bars"></i></a>');

function getRealParent(element) {
	let parent = element.parentElement;
	while(parent != null && parent.parentElement != null && parent.parentElement != document.getElementsByTagName('body')[0]) {
...
$('.toggle').click(function() {
	...
	const menu = getRealParent(this);
	$(toggleButton).addClass('hidden');
	$(menu).addClass('stackable');
	$(menu).find('.item:not(.toggle)').addClass('visible');
	$('.grid').css('filter', 'blur(2px)');

	$(window).click(function(e) {
		if(getRealParent(e.target) != menu) {
			$(toggleButton).removeClass('hidden');
			$(menu).removeClass('stackable');
			$(menu).find('.item:not(.toggle)').removeClass('visible');
			$('.grid').css('filter', 'none');
			$(window).unbind();
		}
	});
});